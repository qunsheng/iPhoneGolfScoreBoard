/************************************************************** 
 * 
 * Golf digital score card javascript libraries
 * 
...
WEBCLOUZ.GOLF.MODEL = (function (model) {
	...
	model.preciseRound = function (value, decPlaces){
	...
	model.calSummary = function(){
		...
		if(model.summary.totalDrive >0){
			var ave = model.summary.fairwayHit/model.summary.totalDrive;
			var aveRound = model.preciseRound(ave, 2);
			model.summary.aveFairwayHit =  parseInt(aveRound  *100);
			console.info("firwayhit", ave, aveRound, model.summary.aveFairwayHit);
		}
		if(model.summary.totalHolePlayed > 0){
			var ave = model.summary.totalPutts/model.summary.totalHolePlayed;
			model.summary.avePuts =  model.preciseRound(ave, 2);
			console.info("avePuts", ave, model.summary.avePuts );
				
			var ave = model.summary.totalPenaltyHoles/model.summary.totalHolePlayed;
			var aveRound = model.preciseRound(ave, 2);	
					
			model.summary.avePenalty = parseInt(aveRound *100);	
			console.info("totalPenaltyHoles", ave, aveRound,  model.summary.avePenalty );
		}
		if(model.summary.total100Less > 0){
			
			var ave = model.summary.made100ToGreen/model.summary.total100Less;
			var aveRound = model.preciseRound(ave, 2);
			model.summary.ave100ToGreen =  parseInt(aveRound  *100);
			console.info("ave100ToGreen ", ave, aveRound, model.summary.ave100ToGreen );
		}else {
...
WEBCLOUZ.GOLF.CONTROLER = (function (controller) {
	...
	controller.displayHole = function (hole) {
		...
		// set img
		//
		var imgString = 'img/h'+ hole.number  + '.png';
